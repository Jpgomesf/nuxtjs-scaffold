  ...
  <v-data-table-server v-model:items-per-page="itemsPerPage" :headers="headers" :items="serverItems"
    ...
    <template v-slot:tfoot>
      <tr>
        ...
        <td>
          <v-text-field v-model="calories" class="ma-2" density="compact" placeholder="Minimum calories" type="number"
            hide-details></v-text-field>
        </td>
      </tr>
    </template>
...
const loadItems = async ({ page, itemsPerPage, sortBy }) => {
  loading.value = true;
  try {
    const { items, total } = await FakeAPI.fetch({ page, itemsPerPage, sortBy, search: { name: name.value, calories: calories.value } });
